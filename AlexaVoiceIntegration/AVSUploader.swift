...
class AVSUploader: NSObject, NSURLSessionTaskDelegate {
    ...
    func start() throws {
        if self.authToken == nil || self.jsonData == nil || self.audioData == nil {
            throw NSError(domain: /*Config.Error.ErrorDomain*/"AlexaVoiceIntegration", code: 10 /*Config.Error.AVSUploaderSetupIncompleteErrorCode*/, userInfo: [NSLocalizedDescriptionKey : "AVS upload options not set"])
        }
                    ...
                    if httpResponse.statusCode >= 200 && httpResponse.statusCode <= 299 {
                        if let responseData = data, let contentTypeHeader = httpResponse.allHeaderFields["Content-Type"] {
                            ...
                            if let b = boundry {
                                ...
                                self.errorHandler?(error: NSError(domain: "AlexaVoiceIntegration" /*Config.Error.ErrorDomain*/, code: 11 /*Config.Error.AVSResponseBorderParseErrorCode*/, userInfo: [NSLocalizedDescriptionKey : "Could not find boundry in AVS response"]))
                            }
                        }
                    } else {
                        ...
                        self.errorHandler?(error: NSError(domain: "AlexaVoiceIntegration" /*Config.Error.ErrorDomain*/, code: /*Config.Error.AVSAPICallErrorCode*/ 12, userInfo: [NSLocalizedDescriptionKey : "AVS error: \(httpResponse.statusCode) - \(finalMessage)"]))
                    }
    ...
    internal func startUpload(alexaAccessToken : String,
                            audioData: NSData,
                            errorHandler: (error:NSError) -> Void,
                            progressHandler: (progress:Double) -> Void,
                            successHandler: (data:NSData) -> Void
                            ) {
        authToken = alexaAccessToken
        
        print("auth token: \(authToken)")
        
        self.jsonData = createMetadata()!
        
        self.audioData = audioData
        
        self.errorHandler = errorHandler
        self.progressHandler = progressHandler
        self.successHandler = { (data:NSData, parts:[PartData]) -> Void in
            
            for part in parts {
                if part.headers["Content-Type"] == "application/json" {
                   //TODO: Print this out or fire an error in response?
                } else if part.headers["Content-Type"] == "audio/mpeg" {
                    successHandler(data: part.data)
                }
            }
        }
